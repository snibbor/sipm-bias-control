...
#define LT8361  //100V rated boost converter with R6=39k
//#define LT8362  //60V rated boost converter with R6=52.3k
...
uint16_t offset = 220;
uint16_t lt836X_dn = 130;  //default voltage for the boost converter, 120 = 59.8V for LT8361
#elif defined(LT8362)
uint16_t gain = 3500, g42 = 3215, g50 = 3870; // Starting Gain Value, Gain Required for 42V, Gain Required for 50V
uint16_t offset = 220;
uint16_t lt836X_dn = 67;  //default voltage for the boost converter, 67 = 52.3V for 8362/8364
#else
#error "Boost converter model not defined"
#endif
...
void setVoltageAndOffset(uint16_t gain, uint16_t offset)
{
  ...
  Serial.print("Analog Write, gain:");
  Serial.print(gain);
  Serial.print(", offset:");
...
//calculate the actual output voltage (in mv) for the LT836X using the default feedback network
float lt836X_voltage_millivolts_float(uint32_t dn)
{
#ifdef LT8361
  //return (1.6E6  + (1.6E6/39.0E3 + (1.6E6 - dn*1E6/1024*1.0V) / 69.8E3))/1E6;  //from ohm's law
  return (1600.000  + ((1600.000/39000 + 1600.000/69800) - dn*3300.000/1024/ 69800 )*1000000);
#else //LT8362
  return (1600.000  + ((1600.000/52300 + 1600.000/69800) - (dn*(3300.000/1024))/69800 )*1000000);
#endif
}
/*uint32_t lt836X_voltage_millivolts(uint32_t dn)
{
...
#else //LT8362
  return (1600000  + ((1600000/52300 + 1600000/69800) - (dn*(3300000/1024))/69800 )*1000000)/1000;
#endif
}*/

//calculate the DAC DN setting for a specific voltage
int32_t lt836X_dn_from_millivolts_float(int32_t mv)
{
  //do calculation in uV to avoid floating point
  //dn = (mv - 65548)/(-46.1698); //inverse fit to the above function
#ifdef LT8361
  float dn = (mv*1000.0f-65548000)/-46170.0f;
#else  //LT8362
  float dn = (mv*1000.0f-55120000)/-46170.0f;
#endif
  return (int32_t)(dn+0.5f);
}

uint16_t lt836X_dn_from_millivolts(int32_t mv)
{
  //do calculation in uV to avoid floating point
  //dn = (mv - 65548)/(-46.1698); //inverse fit to the above function
#ifdef LT8361
  int32_t dn = (mv*1000-65548000)/-46170;
#else  //LT8362
  int32_t dn = (mv*1000-55120000)/-46170;
#endif
 //   sprintf(msgbuf, "lt836X_dn_from_millivolts:  %d from %u mv %d\n",dn, mv, (mv*1000-65548000));
//    Serial.write(msgbuf);
...
void readEEPROM()
{
  ...
  sprintf(msgbuf, "Read 42V gain: %d and 50V gain: %d)\n", rg42, rg50);
  Serial.print(msgbuf);  
...
void usage()
{
  ...
  Serial.println("calibration[?]          The device will set a voltage and ask you to measure it back for calibration purposes");
  ...
  Serial.println("lt836X_voltage[?] DN    Set boost converter voltage DN [0 1023] mapping to [65.5V 18V], default 60V on LT8361");
...
void parse_command(char *str)
{
  ...
  {
    sprintf(msgbuf, "%d (%u mV)\n", gain, millivoltFromGain(gain));
    Serial.write(msgbuf);
  }
  ...
  else if (strcmp(tok, "gain") == 0)
  {
    ...
    //check that gain value won't put the LDO into drop out
    int mv = millivoltFromGain(gain);
    if(lt836X_voltage_millivolts_float(lt836X_dn) - mv < 1000)
    {
      sprintf(msgbuf, "Gain value may exceed boost voltage! %d dn and voltage %u mV\n", gain, millivoltFromGain(gain));
    }
    else
    {
      sprintf(msgbuf, "%d dn and voltage %u mV\n", gain, millivoltFromGain(gain));
    }
  ...
  else if (strcmp(tok, "dac") == 0 || strcmp(tok, "lt8362x") == 0 || strcmp(tok, "lt8361") == 0)
  {
    ...
    // Always reply something:
    //sprintf(msgbuf, "set Arduino dac channel A0 to %d dn (%d millivolts)\n", o, lt836X_voltage_millivolts(o));
    //Serial.write(msgbuf);
    sprintf(msgbuf, "set Arduino dac channel A0 to %d dn (%2.1f volts)\n", o, lt836X_voltage_millivolts_float(o)/1000.0f);
    Serial.write(msgbuf);
  }
  else if (strcmp(tok, "lt836x_voltage") == 0 || strcmp(tok, "lt836X_voltage") == 0 || strcmp(tok, "lt836X_voltage") == 0)
  {
    tok = strtok(NULL, " \n");
    long o = strtol(tok, NULL, 10);
    if (o < 6 || o > 65)
      Serial.println("LT836X voltage (out of range [6 65])");
    else
    {
      lt836X_dn =  lt836X_dn_from_millivolts(o*1000);
      //must write to A alias of the pin or its interpreted as digital
      analogWrite(A0, lt836X_dn);

      //sprintf(msgbuf, "set dac to %d dn (%d volts)\n", lt836X_dn, o);
      //Serial.write(msgbuf);
       sprintf(msgbuf, "set dac to %d dn (%d volts)\n", lt836X_dn_from_millivolts_float(o*1000), o);
  ...
  else if (strcmp(tok, "dac?") == 0 || strcmp(tok, "lt836X_voltage?") == 0)
  {
    //sprintf(msgbuf, "Arduino DAC set to %d mV (%d DNs)\n", lt836X_voltage_millivolts(lt836X_dn), lt836X_dn);
    //Serial.write(msgbuf);

    sprintf(msgbuf, "Arduino DAC set to %2.1f V (%d DNs)\n", lt836X_voltage_millivolts_float(lt836X_dn)/1000, lt836X_dn);
    Serial.write(msgbuf);
  ...
  else if (strcmp(tok, "calibration") == 0)
  {
    ...
    if(1)
    {
      ...
      //turn on the bias gen in case it was off
      on = true;
      analogWrite(A0, lt836X_dn);
  ...
  else if (strcmp(tok, "?") == 0 || strcmp(tok, "name?") == 0)
  {
    Serial.println(NAME);
    #ifdef LT8361
    Serial.print("Configured for: LT8361");
    #else
    Serial.println("Configured for: LT8362");
    #endif

    Serial.print(" (Boost voltage ");
    Serial.print(lt836X_voltage_millivolts_float(1023)/1000.f);
    Serial.print("-");
    Serial.print(lt836X_voltage_millivolts_float(0)/1000.0f);
    Serial.println("V)");
